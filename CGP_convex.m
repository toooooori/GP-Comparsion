...
% CGP — Constrained Gaussian Process with convexity constraint (f''(t) >= 0)
%
% USAGE
%   [mse, nlpd, coverage, width, elapsedTime] = CGP_convex(t1, x, tstar, y_true)

% ----------- Inputs / Shapes -----------
t1    = t1_in(:);        % n×1   training inputs
x     = x_in(:);         % n×1   training observations
tstar = tstar_in(:);     % n*×1  test inputs
n     = numel(t1);
nstar = numel(tstar);
...
cov_ZT = covSEiso([log(ell);log(sigma_f)],t1);
...
epsv = 1e-12;
prob_y = normcdf(zeros(nstar-set_num,1), mu_dS, sqrt(max(diag(cov_dS), epsv)));
...
prob_y = normcdf(zeros(nstar-set_num,1), Z_mu,  sqrt(max(diag(Z_sigma), epsv)));
else
 t2=tstar_g;
 break;
end
...
[Ks1,Ks2,Ks3,KKs2]=covGP2([log(ell);log(sigma_f)],tstar,t2);                 % prior covariance matrix of Z(T*), Z'(S)
