...
if nargin==4
   ...
   K_1 = sf2*exp(-sq_dists(diag(1./ell)*x')./2);                                                  % covariance
   tempK2=sf2*exp(-sq_dists(diag(1./ell)*x',diag(1./ell)*z')./2);
   K_2 = tempK2.*gp_dist(x(:,i)'/ell(i),z(:,i)'/ell(i)).*(-1/ell(i));
   tempKK2=sf2*exp(-sq_dists(diag(1./ell)*z',diag(1./ell)*x')./2);
   KK_2= tempKK2.*gp_dist(z(:,i)'/ell(i),x(:,i)'/ell(i)).*(1/ell(i));
   tempK3=sf2*exp(-sq_dists(diag(1./ell)*z')./2);
   K_3=tempK3.*(1/ell(i)^2).*(1-sq_dists(z(:,i)'/ell(i)));
