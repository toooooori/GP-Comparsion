function [post nlZ dnlZ] = infExactW(hyp, mean, cov, lik, x, y)

% Exact inference for a GP with Gaussian likelihood. Compute a parametrization
% of the posterior, the negative log marginal likelihood and its derivatives
% w.r.t. the hyperparameters. See also "help infMethods".
%
% Copyright (c) by Carl Edward Rasmussen and Hannes Nickisch, 2013-11-11.
...
L = jitterchol(K+sn2*eye(n));                        % Cholesky decomposition
...
if nargout>1                               % do we want the marginal likelihood?
  ...
  if nargout>2                                         % do we want derivatives?
    ...
    Q = L'\(L\eye(n)) - alpha*alpha';     % precompute for convenience
