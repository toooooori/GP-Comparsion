...
funcs   <- c("cosh","parabola","stepwise_convex")
out_dir <- "results_convex_stats"
if(!dir.exists(out_dir)) dir.create(out_dir)

read_func <- function(func_name){
  files <- c(
    CGP = paste0("results_convex/cgp_runs_", func_name, ".csv"),
    SR  = paste0("results_convex/sr_runs_",  func_name, ".csv"),
    BF  = paste0("results_convex/bf_runs_",  func_name, ".csv")
  )
  ...
  bind_rows(dfl) |>
    mutate(method = factor(method, levels=c("CGP","SR","BF")))
}
...
run_friedman <- function(df, metric){
  wide <- df |>
    ...
    pivot_wider(names_from = method, values_from = !!rlang::sym(metric)) |>
    drop_na()
  mat <- as.matrix(wide[, c("CGP","SR","BF")])
